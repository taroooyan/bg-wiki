...
Nov 24, 2013 – verbose draft needs editing. Adds from ebsite, devices group, app, etc. hopefully excerpts in context, usually without attribution. Community-edited wiki page. (needs more input  photos, log file, LEDs diagrams, etc.  Please check [??]=unsure

Community edited operation manual is for nano users including novices. When this user guide doesn’t answer your Nano question, then surf the resources, read the [[community-edited Nano wiki|https://github.com/Safecast/bGeigieNanoKit/wiki/_pages]], go to support ([[devices group|https://groups.google.com/forum/?hl=en#!forum/safecast-devices]], contact info@safecast.org). 

***Safecast bGeigie Nano Operation Manual***

Contents: Nano operation guide:

01. 	Resources, links, support
...
18. 	Safety tips for radiation detection


01. ** RESOURCES, LINKS, SUPPORT**: A brief orientation to information resources for users of the Safecast bGeigie Nano mobile sensor of ionizing radiation. 
...
**Software, [[Safecast bGeigie Library|https://github.com/Safecast/SafecastBGeigie]]** is the code that runs the bGeigie devices from Safecast. The necessary firmware building blocks, such as SD card logger, GPS parser, power management, etc, are included in the form of an Arduino library.
...
* The battery is detailed in the Nano Manual assembly kit page. An optional extra tool is * USB LiPoly Charger - Single Cell Battery Charger https://www.sparkfun.com/products/10161 
...
* Options, desiderdata.. see the wiki parts list + options page. But quick list includes: 
? GPS antenna option
...
* CONFIG.TXT: default is: 9600,26,3,2. (Change baud according to hardware customization.)
9600	Baud rate
26	Data bits
3	Mark Parity (3=>bit is always logical 1)
2	2	stop bits (disables use of MARK parity)
...
* * Alarm* The alarm threshold is edited in this parameter. A threshold of 100 CPM is given in the [[SAFECAST.TXT archive file |https://github.com/Safecast/bGeigieNanoKit/tree/master/SD%20card]]. (My kit #2108 arrived with AL=150.) The setting can be left blank (turned off), or set up to a maximum 99,999 CPM. [??] 
...
# format=1.1.1nano
# deadtime=on
# nm=YW_123456789,tz=2,cn=US,cpmf=334,st=0,ss=0,sh=100,sm=0
$BNRDD,2108,2013-11-03T12:25:39Z,21,3,413,A,3145.6970,N,03510.2840,E,133.00,A,26738691,0*40

Other Files on the MicroSD Card **
* The micro-SD card is delivered with apple disc system files and folders. [non Apple devices users can delete or leave these on micro-SD? Plenty of space to leave them on the 2gb Micro-SD card.] 
...
Mobile sensor -- the Log mode display also shows height (m), distance traversed (km) and duration of measurement (hh:mm)
The log file UTC date/time, TZ header, GPS coordinates, readings, t
Mounting examples need editing and photos.

 Mobile attachment mounting, incl. custom specials
	Mobile attachment format	page
1.		Car	
2.		bicycle 	
(See photos and film clip with bGeigie Nano with extra options (wireless, air quality sensors, bicycle exercise sensors http://www.youtube.com/watch?feature=player_embedded&v=nHpVq1szhz0 )
Safecast Radiation Discussion Group ›
SAFECAST at Tokyo Maker Faire Nov 3rd-4th - Don't miss it!
https://groups.google.com/forum/?hl=en#!topic/safecast-japan/2DqAoqkWAn8

3.		Carry, bag or pack attachment	
4.		Underwater	
5.		Helicopter drone	
6.		Airplane passenger	http://blog.safecast.org/2013/05/safecast-air-force-drone-program/

7.		Snow	http://blog.safecast.org/2012/02/measuring-radiation-in-snow/ 

8.		Baby carriage	


* Carry -- how to use walking (clip)

* bicycle, motor cycle, etc

* car - photo how to put on a car window (strap) 
movie 0:57 JAM red car earlier, bigger bGeigie model which has straps also on side.. 
Close window on straps. Tie straps to interior anchor, inside bar over window. In a blog there’s mention of child opening back seat window and the bGeigie falling off and recovered without damage. Don’t rely on window lock, anchor strap.
0:53-1:04 sean. “Safecast Short Film in Focus Forward Films semifinals
at 3 minute documentary Adrian Storey 
Nov 14, 2012 http://blog.safecast.org/2012/11/safecast-short-film/

* Plane - photo how to use inside a passenger airplane (strap onto utility tray near a window)
How to mount a bGeigie nano in an airplane: Pieter wrote: >Some of you have asked how to mount a bGeigie [as a passenger in commercial flight]. The attached pictures are self explanatory. In this case the unit remained GPS lock even with the tray in the downward position. The key trick is to get the nano to lock onto the GPS -- this only works as long as you have a window seat and typically requires the unit to be very near to the window to get a good lock and may take a few minutes - after that the unit can be a little further a way (30-40cm) as long as there's a good 'view' from the unit to the window. Airplanes are very well shielded, and the ease of getting a good lock may vary from airplane model to exact seat location. Like use of all electronic equipment, please follow the instructions of the airline during take off, in flight and landing. <
Pieter IMG_7688.jpg104 KB View Download
IMG_7689.jpg

* customized: air drone

* customized: underwater

* Snow. Put nano on paper over snow? Brush snow off. 

* Baby carriage : (Baby cannot open Pelican case but can kick Nano out when you’re not looking Keep Nano tied with strap. On 2nd thought it’s not a baby toy given the Nano’s possible contamination in sampling. Strap to handlebars. The height from ground setting is preset at 1 meter for car window. This can be changed in settings file on sd card.) 
...
legal status: CC0 There web entry by joi. “The legal and ethical reasoning behind using CC0 for Safecast data”. Sept 5, 2011 
...
Functions in github repositorities.. 
...
* Apple app: this Safecast Geiger Bot app works in iOS devices. (Note: the nano’s XBee Bluetooth wireless option requires BLE-enable apple devices, e.g. from iPad 3 up). 
http://www.seeedstudio.com/depot/blebee-v100-p-1632.html

Although there’s talk on the web about working on an android version, the porting of geigerbot into android is unlikely. See “Android / Platform Support Mini-FAQ” in https://sites.google.com/site/geigerbot/docs , The app author Nick Dolezal writes, "The majority of what Geiger Bot does is heavily tied to iOS-specific frameworks. It would almost all have to be rewritten."
...
* Oct 2013. GPS model Adafruit “Ultimate GPS”. Fails on 7 recent builds. Manufacturer support forum thread opened on 28 Oct 2013. http://forums.adafruit.com/viewtopic.php?f=22&t=45198
Also see Safecast devices group threads https://groups.google.com/forum/?hl=en#!topic/safecast-devices/iRlxIvspjkE
...
**CASE ORIENTATION up, down** It’s customary to see the lanyard carabiner as top of case with hinge on left so that power button is on the open right. Which direction is up or down, the pancake sensor goes into the liner’s hole. It’s easier to replace the nano into the case when it’s together inside the liner.  
...
*The case may be rinsed with clean slightly damp microfiber coth. 
-- [Last rinse preferably in filtered or even distilled water-??]
*LED wipe? http://www.photodon.com/Screen_Cleaning_Tips_Photodon.pdf
o Work with the screen display turned off and cool. 
o Wipe the screen using light pressure. 
o Use clean, lint free cotton, microfiber cloths or low-lint wipes. 
(if the LED is dirtied when the Nano is outside its closed case, follow the cleaning rules ,such as http://www.photodon.com/Screen_Cleaning_Tips_Photodon.pdf
. If bird dropping falls on the LeD when you hold the open Nano, see this as a sign of good fortune.) 
...
Old analog clocks had a way to adjust their regulator too, but when have you ever heard of a digital quartz wristwatch needing adjustment? Never? Me neither.
- jam

That said, e.g. example Univ of Maryland, Chapter II, Radiation Instrumentation
http://www.des.umd.edu/rs/material/tmsg/rs4.html
...
validity**Data interpretation, presentation****
* Data sharing, interpretation, reliability, validity, robustness, presentation, mapping, analysis

** Reliability**. Learn to use correctly… 
...
- The MICRO-SD CARD can not be read over the USB port [??] -- instead use a micro SD card reader
...
Nano users include both hardcore nuclear expert technical users and novices, totally new to radiation detection. Although all users are interested in radiation safety, we don’t all begin at the same place. A naïve casual approach may not be hazardous, but it is inappropriate. Take some minutes to surf and read radiation detection safety guidelines. Familiarity with Nano’s operation includes awareness of its safe use. 

Although Safecast may not yet have a separate advisory page on safe use and hazard avoidance, the subject is not taken for granted. Many of its films, blogs, help, threads and links do address safe use. Here is a short list of some of these advisories.
...
Also in Safecast thread, it’s written: >”High zone is not for novice, but requires training…”

*[if this list of safecast safe use is moved to separate wiki page, then to delete this section of operating manual, leave brief pointer]*   
